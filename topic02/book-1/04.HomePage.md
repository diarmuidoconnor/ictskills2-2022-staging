...
~~~
We are using Material's Grid system to achieve a responsive UI. For example, when the browser's viewport is extra-large (xl), we allow six cards per row, whereas a small viewport (sm) allows only two cards per row - see later.

This page will ultimately be plugged into a routing component. In the meantime, we will statically connect it to a top-level component that feeds it an array of movies. Create `src/index.js` and add this code:
~~~
import React from "react";
