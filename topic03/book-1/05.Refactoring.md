...
The bulk of the code currently in `pages/homePage.js` has moved to the template page component above. In `pages/homePage.js`, completely replace its entire content with the following:
```
...
There is another known bug in our app; the movies listed on the 'Favourite Movie' pages each have an 'Add to Favourites' icon, which is not useful. In a later lab we will change it to a 'Trash Bin' icon to allow the user to remove the movie from the favourites.
...
```
The app functionality should all work as before after these changes.
