...
~~~
We are now ready to use react-query in components that perform data fetching. We will temporarily break the favouurite tagging feature in the process, but in the next section, we rectify the problem.
...
const HomePage = (props) => {
  ...
  const favouurites = movies.filter(m => m.favouurite)
